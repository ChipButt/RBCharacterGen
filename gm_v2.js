...
function previewCard(target,kind,item){target.innerHTML=`<img src="${item.image}" alt="${item.name}"><div><strong>${item.name}</strong><span>${kind} — this exact card will be placed onto the GM Story Sheet.</span></div>`}
...
function build(){
 ...
 $('npcName').textContent=npc.name;$('npcRole').textContent=s.problem.role;$('npcTraits').textContent=npc.traits.join(' • ');
 $('npcQuote').textContent=`“${s.problem.quote[0]} ${s.problem.quote[1]}”`;
 $('modeText').textContent=s.usePuzzle&&s.useMonster?'PUZZLE + BATTLE':s.usePuzzle?'PUZZLE':'BATTLE';
 let beats;
 if(s.usePuzzle&&s.useMonster) beats=[
 ...
 $('beatList').innerHTML=beats.map(x=>`<li>${x}</li>`).join('');
 $('pressureList').innerHTML=s.setting.press.slice(0,3).map(x=>`<li>${x}</li>`).join('');
 $('hurryList').innerHTML=HURRY.map(x=>`<li>${x}</li>`).join('');
 $('endingText').textContent=`${s.problem.success}; ${s.setting.ending}.`;
 const cards=[];if(s.usePuzzle)cards.push({name:s.puzzle.name,image:s.puzzle.image});if(s.useMonster)cards.push({name:s.monster.name,image:s.monster.image});
 const area=$('cardsArea');area.className='cards-area '+(cards.length===2?'two':'one');area.innerHTML=cards.map(c=>`<img class="card-img" src="${c.image}" alt="${c.name}" data-full="${c.image}">`).join('');
 $('cardLabels').textContent=cards.map(c=>c.name).join(' • ');$('tapNote').textContent='Tap a card to enlarge';
 area.querySelectorAll('.card-img').forEach(img=>img.addEventListener('click',()=>openCard(img.dataset.full)));
...
$('modalClose').onclick=closeCard;$('cardModal').addEventListener('click',e=>{if(e.target===$('cardModal'))closeCard()});
$('runBtn').onclick=build;$('exitRun').onclick=()=>document.body.classList.remove('run-mode');
refresh();
